...
export type DiamondPools = {
  ...
  "instructions": [
    ...
    {
      "name": "clearPhantomAtBoundary",
      "docs": [
        "C1 — re-anchor the LITE phantom to `physical - books` at a drained state. Cosigned,",
        "admin-only. The only clearing path that exists: `crank_remark_phantom` folds a pure surplus",
        "only, and `initialize` cannot run twice (F42), so without this a campaign inherits its",
        "predecessor's phantom permanently."
      ],
      "discriminator": [
        37,
        4,
      ...
      "accounts": [
        {
          "name": "config",
          "docs": [
            "MUT: the cosign gate consumes the nonce, and D3's latch may write `defensive_mode`."
          ],
          "writable": true,
          "pda": {
            "seeds": [
        ...
        },
        {
          "name": "miningPool",
          "docs": [
            "Read-only: supplies `factor_mirror`, the pinned miner key, the drained-share and",
            "live-position counts, and the post-evacuation flag."
          ],
          "pda": {
            "seeds": [
              {
        ...
        },
        {
          "name": "stakingPool",
          "docs": [
            "MUT — its book is ROLLED to `factor` before being read. `evacuate.rs:670-671` does the same",
            "thing for the same reason: physical, books and the new checkpoint must all be expressed at",
            "ONE factor or the re-anchor bakes in a refining mismatch."
          ],
          "writable": true,
          "pda": {
            "seeds": [
        ...
        },
        {
          "name": "unclaimed",
          "docs": [
            "Read-only, and NOT optional: the unclaimed pot is a real fourth claimant on the shared",
            "miner. `enumerable_books_legs` takes it as a separate argument precisely so this account",
            "cannot be quietly dropped from the frame."
          ],
          "pda": {
            "seeds": [
              {
    ...
    },
    {
      ...
      "accounts": [
        ...
        },
        {
          "name": "feeBucket",
          "docs": [
            "this account is RETIRED — the ClaimSOL-delta channel it split against is structurally",
            "dead under ORE's `auto_return = 1` (see `classify_harvest_sweep`), so nothing routes",
            "here from the harvest anymore. The account stays in the frame (ABI stability) and the",
            "dead branch below stays compiled; per-source provenance returns at genesis-2.",
            "Seeds-validated.",
            "",
    ...
    },
    {
      ...
      "docs": [
        ...
        "",
        "F34 (desk Option A, genesis-2): `executor_cap_grams` bounds the physical claim so",
        "the downstream STAGE front stays affordable to the keeper. `u64::MAX` = uncapped",
        "(byte-identical to the pre-cap law). A cap below MIN_CRYSTALLIZE_TARGET_GRAMS",
        "reverts clean — phase unchanged, nothing claimed, no dust consumption. The",
        "uncapped remainder stays physically in the miner and is carried by the existing",
        "LITE tripwire entry (intended − claimed books as a phantom credit), the same",
        "conservation law the dust path already rides. NOTE: the desk signature named",
        "lamports; grams is deliberate — the program has no SOL↔ORE oracle by design, and",
        "the keeper (which already quotes for the A1 impact cap) converts at its own quote."
      ],
      "discriminator": [
        190,
        213,
    ...
    },
    {
      ...
      "accounts": [
        ...
        },
        {
          "name": "unclaimedCustodyAta",
          "docs": [
            "created once at `initialize` (program-PDA authority, uncloseable, bounded), so it always",
            "exists. ADDRESS-CHECKED IN THE HANDLER, exactly as `exiter_store_ata` two fields up is, and",
            "for a reason that is about the BPF stack rather than about validation:",
            "",
            "⛔ F47. `Box<Account<T>>` does NOT keep the account off the stack — Anchor's generated",
            "`try_accounts` deserializes it into a stack temporary and only then moves it to the heap. All",
            "13 boxed accounts in this struct therefore cost their full size transiently, and the frame",
            "sat at EXACTLY 4096 bytes, the SBF hard limit, with zero headroom. C2's 2-byte append to",
            "`Config` pushed `PayMiningExit::try_accounts` to 4104 and mainnet would have answered with an",
            "AccessViolation during account validation — on the IRREVOCABLE `MINING_EXITS` phase, which",
            "closes `evacuate_claim_all` behind the entire TVL.",
            "",
            "This account is the cheapest possible reclaim: its typed data is NEVER read (it appears only",
            "as `.to_account_info()`, a transfer destination in `record_unclaimed_store`), and nothing",
            "else in the struct references it, so demoting it removes a ~165-byte temporary and generates",
            "no extra validation code. Demoting `store_mint` instead makes the frame WORSE (4104 → 4128),",
            "because three `associated_token::mint = store_mint` constraints then have to be re-derived.",
            "",
            "The check below is behaviourally identical to the `associated_token::` constraint it",
            "replaces: same derivation, still evaluated unconditionally before any use."
          ],
          "writable": true
        },
        {
  ...
  "errors": [
    ...
    {
      "code": 6103,
      "name": "executorCapBelowFloor",
      "msg": "executor cap is below the crystallize floor — fund the keeper or raise the cap; nothing was claimed and the cycle is unchanged"
    },
    {
      "code": 6104,
      "name": "cappedClaimKeeperOnly",
      "msg": "a binding executor cap may only be passed by the keeper — a permissionless crank must pass u64::MAX (uncapped)"
    },
    {
      "code": 6105,
      "name": "phantomClearNotDrained",
      "msg": "a pool still has redeemable shares outstanding — every holder must be out before the phantom may be re-anchored"
    },
    {
      "code": 6106,
      "name": "phantomClearPositionsLive",
      "msg": "a mining position still exists — its uore_base/rore_stock annex is NOT visible in total_shares, and re-anchoring over it would fold that holder's entitlement into a pricing-excluded counter"
    },
    {
      "code": 6107,
      "name": "phantomClearFactorRegressed",
      "msg": "the pool's factor mirror sits BELOW the phantom's own checkpoint — run a freeze first; writing the anchor backwards would make the next roll double-accrue"
    },
    {
      "code": 6108,
  ...
  "types": [
    ...
    },
    {
      ...
      "type": {
        ...
        "fields": [
          ...
          {
            "name": "exitFeeMiningBps",
            "docs": [
              "Mining-exit fee in bps, charged on the SOL leg at `pay_mining_exit`, bounded by",
              "`MAX_EXIT_FEE_BPS`. The mining sibling of `exit_fee_staking_bps`; mining exits were free",
              "before this.",
              "",
              "READ LIVE AT PAY — there is deliberately NO `Order` rate snapshot, and that is not an",
              "oversight. `order.fee_bps_snapshot` is ALREADY occupied on the mining rail: intake writes",
              "`config.perf_fee_bps` into it and `mining_exit.rs` consumes it as the perf rate, so the",
              "`min(live, snapshot)` exiter protection `settle_staking_exit` uses is not available here —",
              "it would compare a 50 bps fee against a 3000 bps perf rate, never bind, and let a future",
              "perf cut silently clamp the exit fee. Adding a SECOND `u16` to `Order` is worse than",
              "unavailable, it is unsafe: `Order` is `init`ed at `8 + Order::INIT_SPACE`, Borsh fails on a",
              "SHORT account, and there is no migration rail for per-user PDAs — so every Order created",
              "before the upgrade would become permanently undeserializable on the irrevocable",
              "`MINING_EXITS` phase, wedging the cascade behind the entire TVL.",
              "",
              "Exposure from reading live is bounded by `MAX_EXIT_FEE_BPS` and one window, and it is real",
              "rather than theoretical: `crank_freeze` sets `phase = FROZEN` and THEN calls `apply_due`, so",
              "a window enters its own cascade under the config that same freeze applied. It is closed",
              "OPERATIONALLY — the ops script must refuse to queue this field while",
    ...
    },
    {
      "name": "miningExitFeeCharged",
      "docs": [
        "genesis-2: the mining-exit fee, charged on the SOL leg at `pay_mining_exit`.",
        "",
        "`clamped` is the field an operator actually needs. The fee is bounded by",
        "`sol_out - perf_fee`, and that bound BINDS in a real case — `perf_fee` can equal `sol_out`",
        "outright — so a charge that came back short is not an anomaly to investigate but the underflow",
        "guard doing its job. Without this flag the only way to tell the two apart is to re-derive the",
        "perf fee off-chain."
      ],
      "type": {
        "kind": "struct",
        "fields": [
          ...
          },
          {
            "name": "solOut",
            "docs": [
              "The gross SOL leg the rate was applied to (`shares * frozen_nps`)."
            ],
            "type": "u64"
          },
          {
          ...
          },
          {
            "name": "feeAmount",
            "docs": [
              "What was actually taken, AFTER the clamp."
            ],
            "type": "u64"
          },
          {
            "name": "perfFee",
            "docs": [
              "The perf fee taken from the same leg, for reconciling `sol_out` in one place."
            ],
            "type": "u64"
          },
          {
            "name": "clamped",
            "docs": [
              "True ⇒ the rate's own figure exceeded `sol_out - perf_fee` and was cut to fit."
            ],
            "type": "bool"
          }
        ]
      ...
      "type": {
        ...
        "fields": [
          ...
          },
          {
            ...
            "docs": [
              "The lamports OUR OWN flows left on the mining authority — the harvest sweep's",
              "exemption watermark. The keeper maintains an operational float there",
              "(`fund_mining_authority`, which DEBITS `sol_in_vault` — relocated depositor",
              "principal), and `keep = max(rent + cushion, this)` keeps it on the authority:",
              "never swept, never booked anywhere. Bumped by `fund_mining_authority`,",
              "re-snapshotted to the post-sweep balance at every harvest (self-correcting",
              "against untracked outflows like checkpoint fees). F20 (23 Aug 2026): the",
              "quarantine split this once fed is retired — everything ABOVE `keep` books as",
              "winnings (see `classify_harvest_sweep` in freeze.rs for why timing carries no",
              "provenance under ORE's `auto_return` push) — but the float exemption is",
    ...
    },
    {
      "name": "phantomReanchored",
      "docs": [
        "C1: the campaign-boundary phantom re-anchor. Emitted by `clear_phantom_at_boundary`.",
        "",
        "DELIBERATELY WIDE, and the width is the point. This instruction permanently rewrites the",
        "conservation counter and, by construction, makes the residual read zero afterwards — so the",
        "event is the ONLY surviving evidence of what the books actually said at the moment of the",
        "write. Carrying only the new legs would leave an auditor unable to distinguish a re-anchor that",
        "tidied dust from one that absorbed a real over-claim, which is precisely the distinction D3",
        "exists to preserve. Every input to the arithmetic is therefore stamped alongside its output."
      ],
      "type": {
        "kind": "struct",
        "fields": [
          ...
          },
          {
            "name": "prevUMag",
            "docs": [
              "The legs as they stood BEFORE the write."
            ],
            "type": "u64"
          },
          {
          ...
          },
          {
            "name": "physicalU",
            "docs": [
              "The inputs the write was derived from, at `factor_checkpoint`."
            ],
            "type": "u64"
          },
          {
          ...
          },
          {
            "name": "residualU",
            "docs": [
              "The residual MEASURED BEFORE the write — after it, both legs are zero by construction."
            ],
            "type": "i128"
          },
          {
          ...
          {
            "name": "absorbedOverClaim",
            "docs": [
              "True ⇒ this re-anchor absorbed a provable over-claim and latched `defensive_mode`",
              "MANUAL-class. False ⇒ it tidied pool-favor dust and escalated nothing."
            ],
            "type": "bool"
          },
          {
